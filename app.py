...
MODELS = {
    ...
    "2": {
        "name": "FLUX Realism 📸",
        "id": "flux-realism",
        "desc": "Photorealistic images"
    },
    "3": {
        "name": "FLUX Anime 🎌",
        "id": "flux-anime",
        "desc": "Anime style images"
    },
    "4": {
        "name": "Turbo ⚡⚡",
        "id": "turbo",
...
def sizes_keyboard():
    ...
    for key, size in SIZES.items():
        buttons.append([{
            "text": f"{size['name']} ({size['w']}×{size['h']})",
...
def webhook():
    try:
        ...
        if "callback_query" in data:
            ...
            if cb_data.startswith("model_"):
                ...
                if key in SIZES:
                    ...
                    telegram_api("answerCallbackQuery", {
                        "callback_query_id": callback_id,
                        "text": f"✅ {size['name']} selected!"
                    })
                    send_message(chat_id,
                        f"✅ Size: <b>{size['name']}</b> ({size['w']}×{size['h']})\n\n"
                        f"💡 Now: /generate your prompt"
                    )
        ...
        if text.startswith('/start'):
            ...
            send_message(chat_id, "📐 <b>Image Size ಆಯ್ಕೆ ಮಾಡಿ:</b>",
...
def status():
    ...
    if result and result.get('ok'):
        ...
        return jsonify({
            ...
            "provider": "Pollinations.ai (Free, no key needed)",
...
@app.route('/')
def index():
    return "🤖 CC Pic Bot v5 (Pollinations.ai) running! Visit /setup to configure webhook."
